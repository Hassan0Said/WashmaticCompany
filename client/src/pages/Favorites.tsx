  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 overflow-x-hidden selection:bg-orange-600 selection:text-white" dir={dir}>
      ...
      <section className="pt-44 pb-16 bg-gradient-to-br from-blue-950 via-blue-900 to-blue-950/90 relative">
        {/* تطابق تام للشبكة الهندسية والخلفية المريحة للعين */}
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          >
            <div>
              ...
              <div className={`w-16 h-1.5 bg-orange-600 rounded-full mb-3 ${language === 'ar' ? 'mr-1' : 'ml-1'}`} />
              <p className="text-blue-100 font-medium">
                {language === 'ar' 
                  ? `لديك ${favorites.length} منتجات في قائمتك المخصصة` 
                  : `You have ${favorites.length} items in your personal list`}
              </p>
            </div>
              ...
              <div className="flex flex-wrap gap-4">
                ...
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <Button 
                      variant="outline" 
                      className="border-blue-400/40 bg-white/10 text-white hover:bg-white/20 hover:text-white rounded-xl h-12 px-5 font-bold transition-all"
                    >
                ...
                {/* زر طلب سعر مجمع باللون البرتقالي */}
                <Button 
                  className="bg-orange-600 text-white font-bold hover:bg-orange-700 rounded-xl h-12 px-6 shadow-md transition-all"
      ...
      <section className="pt-12 pb-32 bg-transparent">
        <div className="container mx-auto px-4">
          <AnimatePresence mode="popLayout">
            {favoriteProducts.length > 0 ? (
              ...
              /* ================= مظهر القائمة الفارغة ================= */
              <motion.div 
                initial={{ opacity: 0, scale: 0.95 }}
                animate={{ opacity: 1, scale: 1 }}
                exit={{ opacity: 0, scale: 0.95 }}
                className="max-w-xl mx-auto text-center py-20 px-6 bg-white rounded-3xl border border-slate-200 shadow-xs relative overflow-hidden"
              >
                {/* تأثير الإضاءة الخلفية */}
                <div className="absolute -top-10 -left-10 w-28 h-28 bg-blue-500/5 rounded-full blur-2xl pointer-events-none" />
                
                {/* أيقونة القلب النابض */}
                <div className="w-20 h-20 bg-slate-50 border border-slate-100 rounded-full flex items-center justify-center mx-auto mb-5 shadow-inner">
                  <motion.div
                    animate={{ scale: [1, 1.06, 1] }}
                    transition={{ repeat: Infinity, duration: 2.5, ease: "easeInOut" }}
                  >
                    <Heart size={38} className="text-orange-600" strokeWidth={1.5} />
                  </motion.div>
                </div>

                <h2 className="text-2xl font-display font-bold text-blue-950 mb-2">
                  {language === 'ar' ? 'قائمة المفضلات فارغة حالياً' : 'Your Favorites List is Empty!'}
                </h2>
                
                <p className="text-slate-500 text-sm md:text-base max-w-sm mx-auto mb-8 leading-relaxed font-medium">
                  {language === 'ar' 
                ...
                {/* زر التصفح البرتقالي */}
                <Button 
                  className="bg-orange-600 text-white font-bold hover:bg-orange-700 px-8 py-6 rounded-xl text-base shadow-md group transition-all"
